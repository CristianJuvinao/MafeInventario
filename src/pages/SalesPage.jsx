...
import {
  DailyBarChart, DonutChart, HorizontalBarChart, ChartLegend,
} from '../components/atoms/ChartComponents';
import {
  ShoppingCart, Search, Plus, Minus, Trash2,
  TrendingUp, DollarSign, Package, BarChart2,
  CheckCircle, History, X, ArrowRight,
} from 'lucide-react';
    ...
    <div className="card" style={{ padding: '18px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-1px', color: color || 'var(--text)' }}>
            {value}
          </div>
  ...
  return (
    <div className="page">
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 24, fontWeight: 800, letterSpacing: '-0.5px' }}>Ventas</h1>
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              boxShadow:  tab === key ? '0 2px 10px var(--accent-glow)' : 'var(--shadow)',
              transition: 'all .14s',
            }}
          >
            <Icon size={15} /> {label}
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                      <div style={{
                        ...
                        <Package size={15} color={cat?.color || 'var(--text3)'} />
                      </div>
            ...
            {cart.length === 0 ? (
              ...
              <>
                    ...
                    <div key={productId} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <input
                            ...
                            style={{
                              ...
                              padding: '2px 4px', fontSize: 13, fontWeight: 700,
                              border: '1px solid var(--border2)',
                        ...
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontWeight: 800, fontSize: 14 }}>{fmt(p.price * qty)}</div>
                ...
                <div style={{ marginBottom: 12 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text2)', marginBottom: 7 }}>Método de pago</div>
                    ...
                    {PAYMENT_METHODS.map(pm => (
                      <button
                        ...
                        style={{
                          ...
                          border: paymentMethod === pm.key ? '2px solid var(--accent)' : '1px solid var(--border2)',
                          background: paymentMethod === pm.key ? 'var(--accent)' : 'var(--surface)',
                          color: paymentMethod === pm.key ? '#fff' : 'var(--text2)',
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
                    ...
                    <span style={{ fontWeight: 700 }}>{fmt(cartTotals.revenue)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
                    <span style={{ color: 'var(--text2)' }}>Costo</span>
                    <span style={{ fontWeight: 700, color: 'var(--red)' }}>{fmt(cartTotals.cost)}</span>
                  </div>
                  <div style={{
                    ...
                    <span style={{ fontWeight: 800, color: 'var(--green)' }}>{fmt(cartTotals.profit)}</span>
                  </div>
                </div>
                ...
                >
                  {saving ? 'Registrando...' : (
                    <><CheckCircle size={16} /> Confirmar venta <ArrowRight size={14} /></>
                  )}
                </button>
              </>
            )}
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
            ...
            <input
              ...
              style={{ padding: '9px 12px', borderRadius: 10, border: '1px solid var(--border2)', background: 'var(--surface)', color: 'var(--text)', fontFamily: 'var(--font-body)' }}
            />
            <input
              type="date"
              value={dateTo}
          ...
          <div className="card" style={{ overflow: 'hidden' }}>
            {filteredSales.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '56px 24px', color: 'var(--text3)' }}>
                <History size={32} style={{ opacity: .3, marginBottom: 12 }} />
                <div style={{ fontWeight: 700, fontSize: 16, color: 'var(--text2)' }}>Sin ventas registradas</div>
                      ...
                      <tr key={s.id}>
                        ...
                        <td>
                          <span className="td-cat">
                            <span className="cat-dot" style={{ background: s.categoryColor }} />
                            {s.categoryName || '—'}
                          </span>
                        </td>
                        <td style={{ fontWeight: 700 }}>
                          {s.qty} <span style={{ color: 'var(--text3)', fontSize: 11 }}>{s.unit}</span>
                        </td>
                        <td style={{ fontWeight: 600 }}>{fmt(s.salePrice)}</td>
                        <td style={{ color: 'var(--red)', fontWeight: 600 }}>{fmt(s.purchasePrice)}</td>
                        <td style={{ color: 'var(--blue)', fontWeight: 700 }}>{fmt(s.revenue)}</td>
                        <td style={{ color: s.profit >= 0 ? 'var(--green)' : 'var(--red)', fontWeight: 700 }}>
                          {fmt(s.profit)}
                        </td>
                        <td>
                          {(() => {
          ...
          {filteredSales.length > 0 && (
            <div style={{ marginTop: 10, fontSize: 12, color: 'var(--text3)' }}>
              {filteredSales.length} venta{filteredSales.length !== 1 ? 's' : ''} · Ganancia total:{' '}
      ...
      {tab === 'stats' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            ...
            <div className="card" style={{ padding: '20px 22px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                <div>
                  <div style={{ fontWeight: 800, fontSize: 15 }}>Ganancias diarias</div>
                  <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 2 }}>Últimos 15 días</div>
                </div>
                <ChartLegend items={[
              ...
              {stats.totalSales === 0 ? (
                <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--text3)', fontSize: 13 }}>Sin datos aún</div>
              ) : (
                <DailyBarChart data={stats.dailyChart} height={200} />
              )}
            </div>
            ...
            <div className="card" style={{ padding: '20px 22px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                <div>
                  <div style={{ fontWeight: 800, fontSize: 15 }}>Productos más vendidos</div>
                  <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 2 }}>Por ingresos acumulados</div>
                </div>
                {stats.topCategories.length > 0 && (
                  <div style={{ width: 52, height: 52, flexShrink: 0 }}>
                    <DonutChart slices={stats.topCategories.map(c => ({ name: c.name, value: c.revenue, color: c.color }))} height={52} />
                  </div>
                )}
              </div>
              {stats.topProducts.length === 0 ? (
                ...
                <HorizontalBarChart
                  items={stats.topProducts.slice(0, 6).map(p => ({
          ...
          {/* By category — donut + table */}
          <div className="card" style={{ padding: '20px 22px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: stats.topCategories.length > 0 ? '200px 1fr' : '1fr', gap: 24, alignItems: 'center' }}>
              {stats.topCategories.length > 0 && (
                <div>
                  <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 10 }}>Distribución</div>
                  <DonutChart slices={stats.topCategories.map(c => ({ name: c.name, value: c.revenue, color: c.color }))} height={160} />
                  <ChartLegend
                    items={stats.topCategories.map(c => ({ label: c.name, color: c.color }))}
                    style={{ marginTop: 12, justifyContent: 'center' }}
                  />
                </div>
              )}
              <div>
                <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 14 }}>Ventas por categoría</div>
                {stats.topCategories.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '24px 0', color: 'var(--text3)', fontSize: 13 }}>Sin datos aún</div>
                ) : (
                  <div className="table-container">
                    <table>
                      <thead>
                        ...
                        {stats.topCategories.map(c => {
                          ...
                          return (
                            <tr key={c.categoryId}>
                              ...
                              <td style={{ color: 'var(--blue)', fontWeight: 700 }}>{fmt(c.revenue)}</td>
                              <td style={{ color: 'var(--green)', fontWeight: 700 }}>{fmt(c.profit)}</td>
                              <td>
                                <span style={{ padding: '3px 9px', borderRadius: 99, background: 'var(--green-dim)', color: 'var(--green)', fontSize: 12, fontWeight: 700 }}>
                                  {margin}%
                                </span>
                              </td>
                            </tr>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
            {[
              { label: 'Ingresos totales',  value: fmt(stats.revenueTotal), color: 'var(--blue)'   },
              { label: 'Ganancia total',    value: fmt(stats.profitTotal),  color: 'var(--green)'  },
              { label: 'Unidades vendidas', value: stats.unitsTotal,        color: 'var(--accent)' },
              { label: 'Ventas registradas',value: stats.totalSales,        color: 'var(--text)'   },
